...
function findIndexByProperty(arr, name) {
  ...
  let foundIndex = (element  => element.name === name)

  if(foundIndex !== undefined){
    return arr.findIndex(foundIndex);
  }else{
    return -1;
  }
}
...
function findLastEvenNumber(arr) {
  ...
  let lastEvenNumber = arr.findLast((element) => element % 2 === 0)
...
function getPositiveArrayValues(arr) {
  ...
  let positiveValue = valuesArray.filter(value => value > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let squaredArray  = flatArray.map(x => x ** 2)
  let filteredSquaredArray = squaredArray.filter(item => item > threshold)
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  let lastElement = arr.slice(-n);
  let sum = lastElement.reduceRight((accumulator, currentValue) => accumulator + currentValue, 0);
  lastElement.forEach(element => {console.log(element)})
  return sum
}
...
function wordLengths(arr) {
  ...
  let lengthsArray = arr.map((element) => element.length);
  return lengthsArray
}
...
function findLongestWord(arr) {
  ...
  let longestWord = arr.reduce((currentLongest, word) => {
    // Порівнюємо довжину поточного слова з довжиною найдовшого слова
    if (word.length > currentLongest.length) {
      return word; // Якщо поточне слово довше, то повертаємо його
    } else {
      return currentLongest; // Інакше повертаємо попереднє найдовше слово без змін
    }
  }, '');
...
function findDuplicateElements(arr) {
  ...
  let duplicateElements = arr.filter((element, index, self) => self.indexOf(element) !== index);
...
function calculateTotalPrice(arr) {
  ...
  let totalPrice = arr.reduce((accumulator, currentItem) => {
...
function findLastNegativeNumberIndex(arr) {
  ...
  let lastIndex = (element) => element < 0;
